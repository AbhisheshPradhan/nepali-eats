# NepaliEats — Page Copy

Drop-in copy for the site, in the project voice (see
[VOICE_AND_TONE.md](VOICE_AND_TONE.md), the governing style guide). No code changes
here. Every line is dash-free and AU-spelled. SEO/UX section references (e.g. "SEO §2",
"UX B3") point at [LAUNCH.md](LAUNCH.md).

Legend: **KEEP** = already on-voice, leave it. **CHANGE** = swap to the proposed line.
**NOTE** = a linked SEO or UX point worth doing when you touch this.

## Contents

- **Part 1 — Homepage & core chrome** (meta, hero, featured, story, header, footer,
  cross-cutting CTAs, Explore microcopy, 404, add-a-spot)
- **Part 2 — Directory money pages** (reusable dish lines, city/state, suburb, tag,
  momo, FAQ, restaurant detail incl. the blurb-generator spec §G1)

---

# Part 1 — Homepage & core chrome
...
- **Near-me button (CHANGE):** `Find the closest momo to me`
    - Current line ("Share your location and we'll find the closest momo") is long and
      wraps into an oval pill on mobile (LAUNCH.md UX B3). Shorter fixes both.
...
- **H2 (CHANGE):** `Where we're eating this week`
    - "This week's hidden gems" leans on "hidden gem", which we are retiring from repeat
      use. The new line is warmer and sounds like a real person.
...
- **NOTE (SEO):** these cards link to `/explore?focus=`. Point them at
  `/restaurant/[slug]` so detail pages get internal links (LAUNCH.md SEO §2).
...
- **"Log in" (CHANGE):** remove it until auth exists. It currently routes to the "coming
  soon" add-a-spot page, so tapping "Log in" and landing there breaks trust (LAUNCH.md
  UX B5). Bring it back as `Save your spots` when login is real.

## Footer

- **Blurb (CHANGE):** `Every plate of Nepali food in Australia, from steamy momo windows to Sunday market stalls.`
- **Add the tagline (CHANGE):** show `Find your momo people.` near the logo. This is its
  home as a recurring brand signature.
- **Column: Explore (CHANGE labels + links).** Real destinations, anchor text that
  matches the target page (LAUNCH.md SEO §2):
    - `Momo` to `/momo`
...
- **Column: Hungry? (CHANGE links).** Today all three point at `/stories`. Point each at
  its real guide once written: `Momo guide`, `What to order`, `Festival eats`.
...
- **Map loading (CHANGE):** `Warming up the map...` (a bit of warmth over "Loading map...")
...
## Summary of homepage changes (everything else is KEEP)
...
The restraint is deliberate. The story block, carousel headings, and 404 are already
strong, so changing them would be churn, not improvement.

---

# Part 2 — Directory money pages

Copy for the SEO money pages: city, suburb, tag, the momo flagship, and the restaurant
detail page. These pages currently run a single intro sentence over a card grid
(LAUNCH.md SEO §4). To rank they need real content. This gives you the copy; the
template needs new slots for it (intro paragraphs, a "what to order" block, an FAQ, and
...
- **Thakali dal bhat:** the Thakali thali. Black dal, rice, gundruk and a spread of
  sides, with refills until you tap out.
- **Sel roti:** a sweet, ring-shaped rice bread, crisp outside and soft within.
- **Newari (choila, bara, samay baji):** the Newari table, smoky choila, lentil bara and
  the samay baji spread.
- **Tibetan (thukpa, laphing):** Tibetan-Nepali warmers, thukpa noodle soup and cold,
  springy laphing.
...
- **Cross-links (NOTE, LAUNCH.md SEO §2/§6):** below the grid, link to the suburbs in
...
- **Cross-links (NOTE):** "More Nepali food near {Suburb}" linking to the parent city
  page and 2-3 neighbouring suburbs.

## D. Tag pages (`/tag/[tag]`)

Each tag gets its own intro (2-3 sentences) plus the dish line from section A. Replace
the current one-liners.
...
Three questions each, answered honestly and usefully. Supports FAQ schema and AI answers
(LAUNCH.md SEO §4/§5).
...
The current blurb is one fixed sentence reused on every page, which reads as templated
thin content (LAUNCH.md SEO §3). Replace it with a small generator that varies the
sentence and only includes clauses when the data exists. This is a spec plus copy, not
code.
...
- The meta description uses a compact one-sentence version, not the full blurb, so the
  snippet and the on-page text are not identical.
...
- **More spots block (KEEP copy):** `More spots in {suburb}`
    - **NOTE (LAUNCH.md SEO §2):** point this at `/nepali-restaurants/[suburb]`, not
      `/explore?suburb=`. Add a sibling link to the state page.

## What needs a template change to hold this copy

The copy above assumes these new slots on the listing template (ListingGrid) and detail
page:

1. Listing pages: second intro paragraph, a "what to order" block, an FAQ block, and a
   cross-links row.
2. Detail page: a "nearby spots" mini-list, and the richer blurb generator.
3. FAQ + ItemList JSON-LD to match the visible FAQ and list (LAUNCH.md SEO §5).
