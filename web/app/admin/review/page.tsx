...
const MODES: { key: ReviewMode; label: string; hint: string }[] = [
  { key: "review_needed", label: "Review needed", hint: "Confirm Nepali or remove false positives" },
  { key: "no_photo", label: "No photo", hint: "Spots missing all photos" },
  { key: "has_photo", label: "Has photo", hint: "Eyeball for wrong/bad photos" },
  ...
  return (
    <div className="max-w-[1180px] mx-auto px-6 py-8">
      <div className="flex items-baseline justify-between gap-4 mb-1">
        <h1 className="font-display font-extrabold text-2xl text-ink-900">Admin · review</h1>
        <Link href="/admin" className="text-sm text-chili-600 hover:underline">
          ← Coverage table
        </Link>
      </div>
      <p className="text-ink-500 text-sm mb-5">
        Visual pass for false positives and photo QA. Keep / exclude is reversible; delete is permanent.
      </p>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {MODES.map((m) => {
          ...
          return (
            ...
            >
              {m.label} <span className={active ? "opacity-80" : "text-ink-400"}>{counts[m.key]}</span>
            </Link>
          );
        })}
      </div>
